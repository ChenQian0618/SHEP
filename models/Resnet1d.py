...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=1, out_channel=10, zero_init_residual=False):
        ...
        self.layer1 = self._make_layer(block, base_c, layers[0])
        self.layer2 = self._make_layer(block, base_c*2, layers[1], stride=2)
        self.layer3 = self._make_layer(block, base_c*4, layers[2], stride=2)
        self.layer4 = self._make_layer(block, base_c*8, layers[3], stride=2)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Linear(base_c*8 * block.expansion, out_channel)
...
if __name__ == '__main__':
    ...
    # input = torch.randn(1, 1, 1024).to(torch.device('cuda'))
    # output = models(input)
