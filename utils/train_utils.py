...
from datasets.datasets_utils.generalfunciton import indexed_dataset
...
class train_utils(object):
    ...
    def setup(self):
        ...
        # 读取数据， 输入数据路径和正则化方法， 返回训练数据和测试数据的tuple
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, args.max_epoch):
            ...
            if self.lr_scheduler is not None:
                ...
                logging.info('current lr: {}'.format([float('%.8f'%item) for item in self.lr_scheduler.get_last_lr()]))
            else:
            ...
            for phase in ['train', 'val']:
                ...
                batch_count = 0
                self.c_matrix[phase] = np.zeros_like(self.c_matrix[phase]) # confusion matrix
                ...
                for batch_idx, (inputs, labels_, info) in enumerate(self.dataloaders[phase]):
                    ...
                    if phase == 'train':
                        ...
                        self.MinorRecords['train_acc'].append(correct/inputs.size(0))
                ...
                self.Records["%s_confusion_matrix" % phase].append(self.c_matrix[phase])
        ...
        # After train
        self.Records.update({'Dataset_num_classes': self.Dataset_num_classes, 'Dataset_Fs': self.Dataset_Fs, 'label_name': self.label_name})
        ...
        # check feature save
        if args.flag_save_feature:
            self.save_feature(save_best_data['state_dict'])

        # check data save
        if self.args.save_N_data > 0:
            self._collect_data(self.args.save_N_data)

    def save_feature(self,state_dict,Target_number=100):
        self.model.load_state_dict(state_dict)
        self.model.eval()
        # prepare
        for phase in ['train', 'val']:
            feature_dataset = indexed_dataset(self.datasets[phase], Target_number)
            feature_dataset_loader = torch.utils.data.DataLoader(feature_dataset, batch_size=self.args.batch_size,
                                                               shuffle=False,num_workers=self.args.num_workers,
                                                               pin_memory=(True if self.device == 'cuda' else False))
            # begin to get feature
            outputs,labels =[],[]
            for batch_idx, (input, label, *info) in enumerate(feature_dataset_loader):
                input = input.to(self.device)
                labels.append(label.tolist())
                with torch.no_grad():
                    logits, output = self.model(input, verbose=True)
                    outputs.append(output.detach().cpu().numpy())
            self.feature_data = {'feature':np.concatenate(outputs).squeeze(),
                                 'label':np.concatenate(labels).squeeze()}


            # save feature data
            feature_data_path = os.path.join(self.save_dir, f'feature_data-{phase:s}.mat')
            io.savemat(feature_data_path, self.feature_data)
    ...
    def _collect_data(self, Target_number=100):
        ...
        for temp_phase in ['train', 'val']:  # do both
            ...
            temp_data_path = os.path.join(self.save_dir, f'all_dataset_{temp_phase:s}_SavedInTraining.pkl')
            data, label, infos = [], [], []
    ...
    def plot_save(self):
        ...
        for phase in ['train','val']:
            ...
            sns.heatmap(self.c_matrix_best[phase].astype(int), annot=True, ax=ax, fmt="d",
                        ...
                        linewidths=0.3, annot_kws={'size': 12})  # Blues | RdBu_r | light:b
            ax.invert_yaxis()
        ...
        ax1.plot(epochs, self.Records['train_loss'], "b", markersize=2, linewidth=2)
        ax1.plot(epochs, self.Records['val_loss'], "r", markersize=2, linewidth=2)
        ...
        fig = plt.figure(figsize=[5, 4])
        ax1 = fig.add_subplot(2, 1, 1)
        ax1.plot(steps, self.MinorRecords['train_loss'], "b-", markersize=3, linewidth=1)
        # ax1.grid()
        # ax1.legend('train loss', loc=5)
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('Loss')
        ax2 = fig.add_subplot(2, 1, 2)
        ax2.plot(steps, self.MinorRecords['train_acc'] * 100, "b-", markersize=3, linewidth=1)
        ...
        A = np.array([collected_data_for_spectrum[k].squeeze() for k in temp_label])
        ...
        if self.args.data_type == 'time':
            ...
            # water fall (time)
            fig, (ax1) = plt.subplots(figsize=(5, 4), subplot_kw={'projection': '3d'})
            ...
            # X, Y, Z = axes3d.get_test_data(0.05)
            N = Z.shape[1]//2
            a = ax1.plot_wireframe(X[:,:N], Y[:,:N], np.abs(Z[:,:N]), rstride=1, cstride=0) #
            a.set_linewidth(0.5)
            ...
            fig.savefig(os.path.join(self.save_dir_sub, "data_spectrum-waterfall.jpg"))
