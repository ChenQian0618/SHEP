...
def ExtractInfo(filepath,append_acc = True):
    ...
    with open(filepath, 'r') as f:
        temp = f.readline()
        flag_temp = True
        while '-----Epoch' not in temp:  # 读取参数 直到遇见第一个Epoch
        ...
        while temp != '':  # 进入循环
            if '-----Epoch' in temp:  # 循环开始
                ...
                while "<tempinfo>" in temp:  # 跳过tempinfo
                    temp = f.readline()
            ...
            if '<training time>: ' in temp:  # 获取记录的时间
                record_time = float(temp.split('<training time>: ')[1].split(' ')[0].replace(',', ''))
            if 'Accuracy of recon prototype on val dataset: ' in temp:  # 获取记录的时间
                Params['ReconProtoValAcc'] = float(temp.split('Accuracy of recon prototype on val dataset: ')[1].split(' ')[0].strip(', ')) *100
            temp = f.readline()

    for key in Dict.keys():  # 字典值转换为array
        if key != 'params':
...
if __name__ == '__main__':
    a = ExtractInfo(r'E:\7-SoftwareResult\2-Result_of_ProtoNet\10-Analysis-Explore\DCDS_P\ProtoAE_back-DCDS_P-fft3-1030-200735\training.log', append_acc=True)
    print(1)
