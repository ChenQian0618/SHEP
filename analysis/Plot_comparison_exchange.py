...
import numpy as np
from utils.plot_func import setdefault, color_modify_HSV
import os
...
def plot_comparison_exchange(signal_len=2000, Fs=10e3, dpi=300):
    # frequency
    x_len = signal_len // 2 + 1
    # init
    file_dir = r'E:\OneDrive - sjtu.edu.cn\6-SoftwareFiles\GitFiles\0-个人库\03-科研\2024-PerturbationNet\checkpoint\SA-Combine\CNN-Simulation-time-SNR0-1211-215743\PostProcess_of_FullAnalysis'
    ...
    datas = datas[:, -1, :, :, :]  # (Method, Len, c_b, c) # only use the last c_analyse
    ...
    colors[1:, :] = color_modify_HSV(colors[1:, :], h=0, s=-0.3, v=0)
    plt.close('all')
    figsize = np.array([8.5 / 2.54, 8 / 2.54])
    ...
    for i in range(datas.shape[0]):
        for j in range(datas.shape[1]):
            ...
            ax.set_xlabel('Spectral Freq. (kHz)', labelpad=-15)
            ax.set_yticks(range(len(class_names)), class_names)
            ax.set_ylabel('Background class', labelpad=-15)
    gs.tight_layout(fig, rect=[0, 0.02, 1, 1.01], h_pad=0, w_pad=0.5, pad=0.5)
    fig.savefig(os.path.join(file_dir,'comparison_exchange') + '.jpg', dpi=dpi)
    fig.savefig(os.path.join(file_dir,'comparison_exchange') + '.pdf', dpi=dpi)
